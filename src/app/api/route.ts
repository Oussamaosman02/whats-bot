  ...
  return ok({
    ...
    endpoints: {
      ...
      groups: [
        ...
        "PATCH /api/groups/:jid {subject?, description?, setting?, settings?: {welcomeBrief, welcomeDays}}",
        ...
        "GET /api/groups/:jid/digest (scheduled bulletin)",
        "PUT /api/groups/:jid/digest {hours: ['06:00','14:00','22:00'], audio?, style?: brief|bullets|detailed, enabled?, minMessages?}",
        "DELETE /api/groups/:jid/digest",
        "POST /api/groups/:jid/digest {since?, audio?, force?} (post one now)",
        "GET /api/digests (all scheduled bulletins)",
        "POST /api/groups/:jid/actions {since?, for?: {name, phone}, deliver?} (open tasks / questions / debts)",
      ],
      jobs: ["GET /api/jobs?chat=&status=pending,done", "POST /api/jobs {chat, text, when: 'mañana 9:00' | dueAt, kind?: message|reminder, mentions?, recurrence?}", "GET /api/jobs/:id", "DELETE /api/jobs/:id (cancel)", "POST /api/jobs/:id (deliver now)"],
